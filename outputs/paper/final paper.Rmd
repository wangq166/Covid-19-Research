...
cleaned_data <-read_csv("C:/Users/Lenovo/Desktop/covid-19 paper/inputs/data/cleaned_data.csv")

library(skimr)
# skim(cleaned_data) 
#I use skim to check the characteristics and distribution of each variables, but it cannot be knitted.
...
table5 <- cleaned_data %>%
  filter(`Ever_Incubated` == "Yes"|`Currently_Incubated`=='Yes') %>%
  group_by(`Age_Group`)%>%
  summarize(count = n()) %>%
  mutate(prop = round(count/age_groups,3))
